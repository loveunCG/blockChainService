...
func GetInvitationFromRelay(uri *url.URL, id dappboxprotocol.DeviceID, certs []tls.Certificate, timeout time.Duration) (protocol.SessionInvitation, error) {
	if uri.Scheme != "relay" {
...
func TestRelay(uri *url.URL, certs []tls.Certificate, sleep, timeout time.Duration, times int) bool {
	id := dappboxprotocol.NewDeviceID(certs[0].Certificate[0])
