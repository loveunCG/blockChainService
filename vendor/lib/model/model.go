...
import (
	...
	"github.com/dappbox/dappbox/lib/scanner"
	"github.com/dappbox/dappbox/lib/stats"
	"github.com/dappbox/dappbox/lib/sync"
	"github.com/dappbox/dappbox/lib/versioner"
...
func (m *Model) ClusterConfig(deviceID protocol.DeviceID, cm protocol.ClusterConfig) {
	...
	m.pmut.RUnlock()
	if !ok && !deviceID.IsValidEthereumAddress() { // TODO case split this code path
	...
	dropSymlinks := false
	// TODO remove
	/*l.Warnln("hello.ClientName == ", hello.ClientName)
	l.Warnln("m.clientName == ", m.clientName)
	l.Warnln("hello.ClientVersion == ", hello.ClientVersion)
	l.Warnln("deviceID == ", deviceID)
	...
	for _, folder := range cm.Folders {
		// Fake folder, only used for introduction purposes
		if folder.ID == "" {
			continue
		}
...
func (m *Model) handleIntroductions(introducerCfg config.DeviceConfiguration, cm protocol.ClusterConfig) (folderDeviceSet, bool) {
	...
	// TODO cleanup & fix logic
	// blockchain derived ClusterConfig should correspond to real
	// folders for now this is just adding all devices from all folders
	// and ClusterConfig contains a fake folder with just the device
	// specified in the contract event
	if introducerCfg.DeviceID.IsValidEthereumAddress() {
		for _, folder := range cm.Folders {
...
func (m *Model) handleDeintroductions(introducerCfg config.DeviceConfiguration, cm protocol.ClusterConfig, foldersDevices folderDeviceSet) bool {
	...
	for _, device := range m.cfg.Devices() {
		// TODO handle removal contract events, for now suppress automatic removal
		// of devices introduced by fake ethereum devices
		if device.IntroducedBy.IsValidEthereumAddress() {
			continue
		}
...
func (m *Model) introduceDevice(device protocol.Device, introducerCfg config.DeviceConfiguration) {
    ...
    EthAddress:   introducerCfg.EthAddress,
	}
...
func (m *Model) OnHello(remoteID protocol.DeviceID, addr net.Addr, hello protocol.HelloResult) error {
	...
	if !ok {
        for { // Wait until Ethereum return the address
            if BoolEthAccountAddressModel { break }
...
func (m *Model) generateClusterConfig(device protocol.DeviceID) protocol.ClusterConfig {
	...
	for _, folder := range folders {
		...
		protocolFolder := protocol.Folder{
			ID:                 folder,
			Label:              folderCfg.Label,
			Categories:					folderCfg.Categories,
		...
		for _, device := range m.folderDevices.sortedDevices(folder) {
			...
			protocolDevice := protocol.Device{
				...
				EthAddress:  deviceCfg.EthAddress,
			}
