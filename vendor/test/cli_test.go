...
func TestCLIReset(t *testing.T) {
	...
	cmd := exec.Command("../bin/dappbox", "-no-browser", "-home", "h1", "-reset")
	...
	dirs, err = osutil.Glob("*.dappbox-reset-*")
	if err != nil {
		t.Fatal(err)
	}
	removeAll(dirs...)
...
func TestCLIGenerate(t *testing.T) {
	...
	cmd := exec.Command("../bin/dappbox", "-no-browser", "-generate", "home.out")
	cmd.Stdout = os.Stdout
	cmd.Stderr = os.Stdout
	err = cmd.Run()
	if err != nil {
...
func TestCLIFirstStartup(t *testing.T) {
	...
	cmd := exec.Command("../bin/dappbox", "-no-browser", "-home", "home.out")
