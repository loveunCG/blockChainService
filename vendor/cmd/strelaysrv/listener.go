...
var (
	...
	outboxes       = make(map[dappboxprotocol.DeviceID]chan interface{})
...
func protocolConnectionHandler(tcpConn net.Conn, config *tls.Config) {
	...
	id := dappboxprotocol.NewDeviceID(certs[0].Raw)
	...
	for {
		...
		case message := <-messages:
			...
			case protocol.ConnectRequest:
				requestedPeer := dappboxprotocol.DeviceIDFromBytes(msg.ID)
				outboxesMut.RLock()
