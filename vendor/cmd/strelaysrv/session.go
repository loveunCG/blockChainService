...
import (
	...
	dappboxprotocol "github.com/dappbox/dappbox/lib/protocol"
	"github.com/dappbox/dappbox/lib/relay/protocol"
)
...
func newSession(serverid, clientid dappboxprotocol.DeviceID, sessionRateLimit, globalRateLimit *rate.Limiter) *session {
...
func dropSessions(id dappboxprotocol.DeviceID) {
	sessionMut.RLock()
...
func hasSessions(id dappboxprotocol.DeviceID) bool {
	sessionMut.RLock()
	has := false
	for _, session := range activeSessions {
		if session.HasParticipant(id) {
...
type session struct {
	...
	serverkey []byte
	serverid  dappboxprotocol.DeviceID

	clientkey []byte
	clientid  dappboxprotocol.DeviceID
...
func (s *session) HasParticipant(id dappboxprotocol.DeviceID) bool {
