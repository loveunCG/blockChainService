...
type Wrapper struct {
	...
	categoryMap map[string]CategoryConfiguration
...
func (w *Wrapper) Categories() map[string]CategoryConfiguration {
	w.mut.Lock()
	defer w.mut.Unlock()
	if w.categoryMap == nil {
		w.categoryMap = make(map[string]CategoryConfiguration, len(w.cfg.Categories))
		for _, cat := range w.cfg.Categories {
			w.categoryMap[cat.Name] = cat
		}
	}
	return w.categoryMap
}

// SetFolder adds a new folder to the configuration, or overwrites an existing
// folder with the same ID.
func (w *Wrapper) SetCategory(cat CategoryConfiguration) error {
	w.mut.Lock()
	defer w.mut.Unlock()
	...
	for i := range newCfg.Categories {
		if newCfg.Categories[i].Name == cat.Name {
			newCfg.Categories[i] = cat
			replaced = true
			break
		}
	}
	if !replaced {
		newCfg.Categories = append(w.cfg.Categories, cat)
	}

	return w.replaceLocked(newCfg)
}

func (w *Wrapper) Category(name string) (CategoryConfiguration, bool) {
	w.mut.Lock()
	defer w.mut.Unlock()
