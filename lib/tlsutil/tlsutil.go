...
func NewCertificate(certFile, keyFile, tlsDefaultCommonName string, tlsRSABits int) (tls.Certificate, error) {
	fmt.Println("Entering tlsutil.NewCertificate")
	var priv interface{}
	var err error
	if tlsRSABits > 0 {
		fmt.Println("rsa.GenerateKey")
		priv, err = rsa.GenerateKey(rand.Reader, tlsRSABits)
	} else {
		fmt.Println("ecdsa.GenerateKey started")

		var curb = elliptic.P384()
		var randomReader = rand.Reader
		priv, err = ecdsa.GenerateKey(curb, randomReader)
		fmt.Println("ecdsa.GenerateKey ended")
	...
	fmt.Println("x509.CreateCertificate")
